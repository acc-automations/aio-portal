...
handlechange(item){
  ...
  if(JSON.parse(item) > 12){
    ...
    //MME LINES
    this.extrathismonth = (this.fiscalmonth+1);
     this.MMErevenue = this.allthevalues
   .filter(mme => mme.section === 'MME' && mme.parentcategorynm === 'TotalRevenue' && mme.fiscalyear == this.fiscalyear && mme.fiscalmonth >= this.fiscalmonth && JSON.parse( mme.fiscalmonth) <= this.fiscalmonth)
     .map(mmeAmount => JSON.parse(mmeAmount.amount))
      .reduce((a,b) => a+b);    

      //console.log(this.fiscalmonth);
      this.MMEcontractcost = this.allthevalues
      .filter(mme => mme.section === 'MME' && mme.fiscalyear === this.fiscalyear && mme.fiscalmonth >= this.fiscalmonth &&  mme.fiscalmonth <= this.fiscalmonth  && mme.parentcategorynm === 'DirectCosts' ||  mme.parentcategorynm === 'OtherContractCosts')
       .map(mmeAmount => JSON.parse(mmeAmount.amount))
       .reduce((a,b) => a+b);
       this.MMErevenue = JSON.stringify(Math.floor(JSON.parse(this.MMErevenue)/1000) + 1);
            this.MMEcontractcost = JSON.stringify(Math.floor(JSON.parse(this.MMEcontractcost)/1000) + 1);
...
//SAPLINES
    ...
    //FOR Months
      if(JSON.parse(item) > 8){
        ...
        this.MMErevenue = this.allthevalues
        .filter(mme => mme.section === 'MME' && mme.parentcategorynm === 'TotalRevenue' && mme.fiscalyear == this.fiscalyear && mme.fiscalmonth == this.fiscalmonth)
          .map(mmeAmount => JSON.parse(mmeAmount.amount))
            .reduce((a,b) => a+b);    

            //console.log(this.fiscalmonth);
            this.MMEcontractcost = this.allthevalues
            .filter(mme => mme.section === 'MME' && mme.fiscalyear == this.fiscalyear && mme.fiscalmonth == this.fiscalmonth && (mme.parentcategorynm === 'DirectCosts' ||  mme.parentcategorynm === 'OtherContractCosts'))
            .map(mmeAmount => JSON.parse(mmeAmount.amount))
            .reduce((a,b) => a+b);
            this.MMErevenue = JSON.stringify(Math.floor(JSON.parse(this.MMErevenue)/1000) + 1);
            this.MMEcontractcost = JSON.stringify(Math.floor(JSON.parse(this.MMEcontractcost)/1000) + 1);
